...
#' @export
check_dataframe_covariate <- function(df, covariate, response = "Count", error = TRUE){
  ...
  output <- check_dataframe_variable(df = df[1, ], variable = all.vars(formula), error = error)
  return(output)
}
