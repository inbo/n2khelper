...
#' @importFrom methods setMethod
#' @importFrom git2r commit push
#' @importFrom utils sessionInfo
setMethod(
  f = "auto_commit",
  signature = signature(connection = "gitConnection"),
  definition = function(package, connection, push = FALSE, ...){
    ...
    if (isTRUE(push)) {
      ...
      tryCatch(
        push(connection@Repository, credentials = connection@Credentials),
        error = function(e){
          warning(e)
        }
      )
