...
describe("write_delim_git()", {
  ...
  it("stops if connection is not a git repository", {
    expect_that(
      write_delim_git(x = x, file = file, local.path = local.path, connection = connection),
  ...
  it("stops if x is not a data.frame", {
    expect_that(
      write_delim_git(x = matrix(0), file = file, local.path = local.path, connection = connection),
  ...
  it("stages the file", {
    ...
    write_delim_git(x = x1, file = file, local.path = local.path, connection = connection)
    expect_that(
